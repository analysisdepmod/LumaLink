...
List<int> sourceIndices(int seed, int k) {
  if (seed >= 1 && seed <= k) return <int>[seed - 1];
  final random = _FountainRandom(seed & _u32Mask);
  final degree = _degree(random.nextDouble(), k);
...
class FountainDecoder {
  FountainDecoder(this.k, this.chunkSize)
  ...
  bool addFrame(int seed, Uint8List data) {
    ...
    final payload = Uint8List(chunkSize)..setRange(0, math.min(chunkSize, data.length), data);
    final remaining = <int>{};
    for (final index in sourceIndices(seed, k)) {
