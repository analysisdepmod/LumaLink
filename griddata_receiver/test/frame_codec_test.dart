...
void main() {
  test('matches the web sender golden frame byte-for-byte', () {
    final packed = encodeFrame(
      DecodedFrame(type: frameData, seed: 0x12345678, payload: Uint8List.fromList([7, 1, 9, 2, 8])),
      128,
      0.6,
    );
    const expected = '';
    final actual = packed.map((byte) => byte.toRadixString(16).padLeft(2, '0')).join();

    expect(actual, expected);
  });

  test('decodes the interleaved, whitened LDPC frame contract', () {
    final sent = DecodedFrame(type: frameData, seed: 0x12345678, payload: Uint8List.fromList([7, 1, 9, 2, 8]));
    const capacity = 128;
