...
int _anchorCount(List<int> bits) => bits.fold(0, (sum, value) => sum + (1 << value));
...
int multiColorCapacityBytes(Color8Grid grid, GridEncoding encoding) {
  ...
  final usable = grid.gridWidth * grid.gridHeight - grid.gridWidth * barcodeRows - _anchorCount(bits);
...
Float64List softDemodulateMultiColor(Color8Grid grid, GridEncoding encoding) {
  ...
  var out = 0;
  for (var cell = anchor; cell < grid.gridWidth * grid.gridHeight && out < output.length; cell++) {
    final reliability = grid.reliability[cell] < 0 ? 0.0 : (grid.reliability[cell] > 1 ? 1.0 : grid.reliability[cell]);
